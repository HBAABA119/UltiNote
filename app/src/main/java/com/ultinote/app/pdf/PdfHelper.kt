...
object PdfHelper {
    ...
    // pressureMult comes from Settings (Soft 1.6 / Medium 1.0 / Firm 0.6) so every stylus feels right.
    fun calculatePressureWidth(baseWidth: Float, pressure: Float, toolType: ToolType, pressureMult: Float = 1.0f): Float {
        val p = (pressure.coerceIn(0.05f, 2.5f) * pressureMult).coerceIn(0.05f, 3.0f)
        return when (toolType) {
            ToolType.PEN_FOUNTAIN -> {
                baseWidth * (0.35f + p * 1.5f).coerceIn(baseWidth * 0.3f, baseWidth * 2.4f)
            }
            ToolType.PEN_BRUSH -> {
                baseWidth * (0.25f + p * 2.25f).coerceIn(baseWidth * 0.25f, baseWidth * 3.0f)
            }
            ToolType.PEN_BALLPOINT -> {
                // Ballpoint is intentionally stable, but still breathes a little with pressure.
                baseWidth * (0.75f + p * 0.5f).coerceIn(baseWidth * 0.6f, baseWidth * 1.5f)
            }
            ToolType.HIGHLIGHTER -> baseWidth
            else -> baseWidth * (0.7f + p * 0.6f)
        }
    }
    ...
    suspend fun exportNoteToPdf(
        ...
        try {
            for ((index, pageEntity) in pages.withIndex()) {
                ...
                // 3b. Draw stickers (emoji) + embedded photos
    ...
    private fun drawTemplateOnCanvas(canvas: Canvas, template: com.ultinote.app.data.model.PaperTemplate, w: Float, h: Float) {
        ...
        val dotPaint = Paint().apply {
            color = Color.parseColor("#B9C4BC")
            style = Paint.Style.FILL
            isAntiAlias = true
        }

        when (template) {
            com.ultinote.app.data.model.PaperTemplate.RULED -> {
            ...
            com.ultinote.app.data.model.PaperTemplate.GRID -> {
                var x = 0f
                while (x < w) {
            ...
            com.ultinote.app.data.model.PaperTemplate.CORNELL -> {
            ...
            com.ultinote.app.data.model.PaperTemplate.DOTTED -> {
                var y = 40f
                while (y < h - 20f) {
                    var x = 40f
                    while (x < w - 20f) {
            ...
            com.ultinote.app.data.model.PaperTemplate.BLANK -> { /* clean paper */ }
            com.ultinote.app.data.model.PaperTemplate.PLANNER_WEEKLY -> {
                val colW = w / 7f
                for (i in 0..7) canvas.drawLine(i * colW, 60f, i * colW, h, linePaint)
                canvas.drawLine(0f, 60f, w, 60f, marginPaint)
                var y = 110f
                while (y < h - 30f) { canvas.drawLine(0f, y, w, y, linePaint); y += 30f }
            }
            com.ultinote.app.data.model.PaperTemplate.DARK_PAPER -> {
                // export stays light for print; faint grid only
                var x = 0f
                while (x < w) { canvas.drawLine(x, 0f, x, h, linePaint); x += 28f }
                var y = 0f
                while (y < h) { canvas.drawLine(0f, y, w, y, linePaint); y += 28f }
            }
            com.ultinote.app.data.model.PaperTemplate.ENGINEERING_GRID -> {
                var x = 0f
                while (x < w) { canvas.drawLine(x, 0f, x, h, linePaint); x += 14f }
                var y = 0f
                while (y < h) { canvas.drawLine(0f, y, w, y, linePaint); y += 14f }
                val major = Paint().apply { color = Color.parseColor("#A9B8AC"); strokeWidth = 1.4f; style = Paint.Style.STROKE; isAntiAlias = true }
                var mx = 0f
                while (mx < w) { canvas.drawLine(mx, 0f, mx, h, major); mx += 70f }
                var my = 0f
                while (my < h) { canvas.drawLine(0f, my, w, my, major); my += 70f }
            }
            com.ultinote.app.data.model.PaperTemplate.MUSIC_STAVE -> {
                var y = 120f
                while (y < h - 80f) {
                    for (l in 0 until 5) canvas.drawLine(40f, y + l * 9f, w - 40f, y + l * 9f, linePaint)
                    y += 95f
                }
            }
            com.ultinote.app.data.model.PaperTemplate.PASTEL_WASH -> {
                var y = 60f
                while (y < h - 40f) { canvas.drawLine(0f, y, w, y, linePaint); y += 34f }
            }
        }
    }
